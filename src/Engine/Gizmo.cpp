...
GIZMO_HIT_FLAGS BB::GizmoCollide(const Gizmo& a_gizmo, const float3 a_ray_origin, const float3 a_ray_dir)
{
    ...
    if (BoxRayIntersect(a_gizmo.pos, a_gizmo.pos + float3(scale_size, scale_size, 0.f), a_ray_origin, a_ray_dir))
        return static_cast<uint32_t>(GIZMO_HIT::RIGHT) | static_cast<uint32_t>(GIZMO_HIT::UP);
    if (BoxRayIntersect(a_gizmo.pos, a_gizmo.pos + float3(0.f, scale_size, scale_size), a_ray_origin, a_ray_dir))
        return static_cast<uint32_t>(GIZMO_HIT::UP) | static_cast<uint32_t>(GIZMO_HIT::FORWARD);
    if (BoxRayIntersect(a_gizmo.pos, a_gizmo.pos + float3(scale_size, 0.f, scale_size), a_ray_origin, a_ray_dir))
...
void BB::DrawGizmo(class EntityComponentSystem& a_ecs, const Gizmo& a_gizmo, const GIZMO_HIT_FLAGS a_hits)
{
    ...
    IsLineSelected(lines[0], right_color, static_cast<GIZMO_HIT_FLAGS>(GIZMO_HIT::RIGHT), a_hits);
    lines[1].p0 = a_gizmo.pos;
    lines[1].p1 = a_gizmo.pos + float3(0.f, a_gizmo.arrow_length, 0.f);
    IsLineSelected(lines[1], up_color, static_cast<GIZMO_HIT_FLAGS>(GIZMO_HIT::UP), a_hits);
    lines[2].p0 = a_gizmo.pos;
    lines[2].p1 = a_gizmo.pos + float3(0.f, 0.f, a_gizmo.arrow_length);
    IsLineSelected(lines[2], forward_color, static_cast<GIZMO_HIT_FLAGS>(GIZMO_HIT::FORWARD), a_hits);
    ...
    IsLineSelected(lines[3], right_color, right_up, a_hits);
    IsLineSelected(lines[4], up_color, right_up, a_hits);
    ...
    IsLineSelected(lines[5], up_color, up_forward, a_hits);
    IsLineSelected(lines[6], forward_color, up_forward, a_hits);
    ...
    IsLineSelected(lines[7], forward_color, forward_right, a_hits);
    IsLineSelected(lines[8], right_color, forward_right, a_hits);

...
void ShadowMapStage::Init(MemoryArena& a_arena, const uint32_t a_back_buffer_count)
{
    ...
    for (uint32_t i = 0; i < m_per_frame.size(); i++)
    {
        ...
        shadow_map_img.array_layers = static_cast<uint16_t>(INITIAL_DEPTH_ARRAY_COUNT);
        ...
        shadow_map_img_view.array_layers = static_cast<uint16_t>(INITIAL_DEPTH_ARRAY_COUNT);
...
void ShadowMapStage::ExecutePass(const RCommandList a_list, const uint32_t a_frame_index, const uint2 a_shadow_map_resolution, const DrawList& a_draw_list, const ConstSlice<LightComponent> a_lights)
{
    ...
    BB_ASSERT(shadow_map_count <= INITIAL_DEPTH_ARRAY_COUNT, "too many lights! Make a dynamic shadow mapping array");
    ...
    depth_attach.image_view = GetImageView(pfd.descriptor_index);
    ...
    rendering_info.view_mask = (1 << shadow_map_count) - 1;
...
void ShadowMapStage::UpdateConstantBuffer(const uint32_t a_frame_index, Scene3DInfo& a_scene_3d_info) const
{
    ...
    a_scene_3d_info.shadow_map_count = INITIAL_DEPTH_ARRAY_COUNT;

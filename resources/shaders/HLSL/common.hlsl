...
float3 CalculateDiffuse_impl(const float3 a_light_dir, const float3 a_color, const float3 a_normal, const float3 a_world_pos)
{
...
float3 CalculateSpecular_impl(const float3 a_light_dir, const float3 a_color, const float3 a_normal, const float3 a_view_dir, const float3 a_world_pos, const float a_specular_strength, const float a_shininess)
{
    const float3 view_dir = normalize(a_view_dir - a_world_pos);
...
float3 CalculateLight(const BB::Light a_light, const float3 a_L, const float3 a_V, const float3 a_N, const float3 a_albedo, const float3 a_f0, const float3 a_orm, const float3 a_world_pos)
{
    float3 light_dir;
